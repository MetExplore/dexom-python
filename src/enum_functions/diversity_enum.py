...
from enumeration import EnumSolution, dexom_results
...
def diversity_enum(model, reaction_weights, prev_sol, thr=1e-5, eps=1e-2, obj_tol=1e-3, maxiter=10, dist_anneal=0.995,
                   out_path="enum_dexom", icut=True, only_ones=False, full=False):
    """
...
#         prev_sol, prev_bin = read_solution(args.initial_solution)
#     else:
...
#                                maxiter=args.maxiter, obj_tol=args.obj_tol, dist_anneal=args.dist_anneal, icut=icut,
#                                out_path=args.output, only_ones=False, full=False)
...
if __name__ == "__main__":
    ...
    imat_solution = imat(model, reaction_weights, feasibility=1e-7, timelimit=6000, full=full)
